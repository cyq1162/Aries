/**
*该文件依赖与TY.Core.Utility.js文件
*/
TY.Global.config = TY.Utility.Ajax.post("GetKeyValueConfig", null, null, false, TY.handler.ajax);

//基类的定义
(function (ty) {
    ty.ExcuteEvent = ExcuteEvent;
...
(function ($, ty) {
    ...
    Common.prototype = {
        _Internal: {
            ...
            Search: function () {
                ...
                this.BtnQuery = function () {
                    function Obj() {
                        ...
                        */
                        this.onBeforeExcute = function (searchList) { };
                        this.onExcute = function (dg, btn_query) {
                            var targetForm = btn_query.parents("form");
                            if (targetForm.form("validate")) {
                                var searchJson = ty.Common._Internal.bulderSeachJson(targetForm);
                                ...
                                if (dg.Internal.type == 'datagrid') {
                                    ...
                                    eval("searchList = '" + str + "'");
                                    var data = { searchList: searchList.replace(/!#/g, "'") };
                                    var flag = this.onBeforeExcute(data.searchList);
                        ...
                        */
                        ...
                        this.onExcute = function (dg, btn_reset) {
                            ...
                            setTimeout(function () {
                                ...
                                if (window.onBeforResetQuery && $.isFunction(window.onBeforResetQuery)) {
                                    window.onBeforResetQuery(btn_reset.parents("form")[0]);
                                }
                            }, 100);
                ...
                */
                ...
                this.BtnAdd = function () {
                    function Obj() {
                        ...
                        this.onExcute = function (dg) {
                            ...
                            if (dg.isEditor) {
                                if (endEditing(dg)) {
                                    dg.operator = dg.PKColumn.Editor.add || "Add";
                                    var pkField = dg.Internal.primarykey;
                                    dg.$target.datagrid("appendRow", {});
                                    dg.PKColumn.Editor.editIndex = dg.$target.datagrid('getRows').length - 1;
                                    dg.$target.datagrid('selectRow', dg.PKColumn.Editor.editIndex).datagrid('beginEdit', dg.PKColumn.Editor.editIndex);
                                }

                            } else {
                                ty.Global.DG.operating = dg;
                                ...
                                ty.Utility.Window.open(viewLink, dg.addTitle, false);
                ...
                this.BtnDelBatch = function () {
                    function Obj() {
                        ...
                        this.onExcute = function (dg) {
                            var result = this.onBeforeExcute();
                            ty.Common.onDel(null, null, dg.Internal.id);
                        }
                        this.onAfterExcuted();
                    }
                    return new Obj();
                    ...
                    function Obj() {
                        ...
                        */
                        ...
                        this.onExcute = function (dg) {
                            ...
                            var jsonString = JSON.stringify(ty.Common._Internal.bulderSeachJson(targetForm));
                            ...
                            //window.open(ajaxOptions.href + '?objName=' + objName + '&searchList='+jsonString, '_self');      
                            var iframeName = "framePost";
                            ifrme = $("<iframe>").attr("id", "div_ifrme_template").attr("name", iframeName).css({ display: 'none' });
                            form_export = $("<form>").attr("action", ty.Utility.Ajax.Settings.url).attr("target", iframeName).attr("id", "form_data");
                            var param = {
                                tableName: objName,
                                method: "Export",
                                objName: dg.viewName,
                                searchList: jsonString,
                                mid: mid
                            };
                ...
                this.BtnExportTemplate = function () {
                    function Obj() {
                        ...
                        this.onExcute = function (dg) {
                            ...
                            var iframeName = "framePost";
                            ifrme = $("<iframe>").attr("id", "div_ifrme_template").attr("name", iframeName).css({ display: 'none' });
                            form_export = $("<form>").attr("action", TY.handler.ajax).attr("target", iframeName).attr("id", "form_template");
                            var param = {
                                objName: dg.viewName,
                                tableName: objName,
                                method: "ExcelTemplate",
                                mid: mid
                            };
                            for (var k in param) {
                                form_export.append($("<input>").attr("name", k).val(param[k]).attr("type", "hidden"));
            ...
            bulderSeachJson: function (targetForm) {
                var json = [], reg_date = /<=\s('[^']+')/;
                targetForm.find("[name]:input[type!=button][type!='reset']").each(function () {
                    ...
                    if ($(this).attr("type") == 'hidden' && !$("[comboname=" + $(this).attr("name") + "]").attr("type") && !$("[comboname=" + $(this).attr("name") + "]").attr("multiple")) {
                        op = 'Equal';
                    }
        ...
        Formatter: {
            ...
            configFormatter: function (configKey) {
                return function configFormatter(value, row, index) {
                    ...
                    if (value != undefined && value.toString() != "") {
                        if (configKey && TY.Global.config[configKey]) {
                            result = getConfigName(configKey, value);
                        }
                    }
                    result = TY.Common.Formatter.afterConfigFormatter(configKey, result, row, index);
                    if (result && result.toString().indexOf('<') == -1) {
                        result = TY.Common.Formatter.stringFormatter(result);
                    }
                    return result;
                }
            },
            ...
            objFormatter: function (objName) {
                ...
                return function objFormatter(value, row, index) {
                    ...
                    if (value != undefined && value.toString() != "") {
                        ...
                        if (TY.Global.m_combobox_json && _obj) {
            ...
            pkFormat: function (dg) {
                return function (value, row, index) {
                    ...
                    if (dg.isEditor == true) {
                        ...
                        var strTemplate = '<a class="{0}" title="{1}" dgid="' + dg.Internal.id + '" onClick="TY.Common._Internal.Editor.{2}(this,\'' + dg.Internal.id + '\',\'' + value + '\',' + index + ')"></a>';
                        if (dg.PKColumn.Editor.BtnEdit.hidden != true) {
                            len += 1;
                            strHtml += ty.Utility.StringFormat.execute(strTemplate,"bj","编辑","onEdit");
                        }
                        if (dg.PKColumn.Editor.BtnDel.hidden != true) {
                            len += 1;
                            strHtml += ty.Utility.StringFormat.execute(strTemplate, "sc", "删除", "onDel");
                        }
                        if (dg.PKColumn.Editor.BtnCancel.hidden != true) {
                            len += 1;
                            strHtml += ty.Utility.StringFormat.execute(strTemplate, "cx", "撤销", "onCancel");
                        }
                        if (dg.PKColumn.Editor.BtnSave.hidden != true) {
                            len += 1;
                            strHtml += ty.Utility.StringFormat.execute(strTemplate, "bc", "保存", "onSave");
                        }
                        strHtml = strHtml.replace('$len',len+buttons.length);
                        for (var i = 0; i < buttons.length; i++) {
                            var btn = buttons[i];
                            btn.setAttribute("onClick", "TY.Common.onOpen(this,'" + value + "','" + dg.Internal.id + "',"+index+")");
                           ...
                           (function () {
                               ...
                               for (var i = 0; i < buttons.length; i++) {
                                   ...
                                   if (btn.className != 'sc') {
                                       ...
                                       btn.setAttribute("onClick", "TY.Common.onDel(this,'" + value + "','" + dg.Internal.id + "'," + index + ")");
                                   }
            ...
            formatEditor: function (row, dg) {
                ...
                if (row.formatter && row.formatter.indexOf('#') != -1) {
                    ...
                    if (configKey) {
                        data = TY.Global.config[configKey] || [];                         
                    }
                    if (objName) {
                        each: for (var i = 0; i < TY.Global.m_combobox_json.length; i++) {
                            if (TY.Global.m_combobox_json[i][objName]) {
                                data = TY.Global.m_combobox_json[i][objName];
                                break each;
                            }
                        }
                    ...
                    settings.options = {
                        ...
                        onSelect: function (record) {
                            var currentEditor = dg.$target.datagrid("getEditor", { index: dg.editIndex, field: row.field });
                            var rowEditors = dg.$target.datagrid("getEditors", dg.editIndex);
                                ...
                                if (rowEditors[i].type == 'combobox') {
                                    ...
                                    if (_parentObjName && _parentObjName == _selfObjName) {
                                        ...
                                        (function (key, parent_id) {
                                            ...
                                            f1: for (var j = 0; j < TY.Global.m_combobox_json.length; j++) {
                                                if (TY.Global.m_combobox_json[j][key]) {
                                                    all_array_children = TY.Global.m_combobox_json[j][key];
                                                    break f1;
                                                }
                                            }
            ...
            formatHeader: function (dg) {
                var json_data = ty.Utility.cloneArray(dg.Internal.headerData, true);
                ...
                if (_post_combox_data.length > 0) {
                    var _post_data = { item_json: JSON.stringify(_post_combox_data) };
                    if (TY.combobox_params && $.type(TY.combobox_params) == "object") {
                        _post_data = $.extend({}, _post_data, TY.combobox_params);
                    }
                    TY.Global.m_combobox_json = TY.Global.m_combobox_json.concat(TY.Utility.Ajax.post("GetCombobox", "objName", _post_data, false));
                }
                ...
                each: for (var i = 0; i < json_data.length; i++) {
                    ...
                    if (i == 0 && (json_data[i].formatter == undefined || json_data[i].formatter == "#" || json_data[i].formatter == "")) {
                        frozen.push({ align: 'center', checkbox: dg.isShowCheckBox, hidden: !dg.isShowCheckBox, field: 'ckb', rowspan: json_data[i].rowspan });
                        json_data[i].formatter = this.pkFormat(dg);
                        if (json_data[i].width < 36) {
                            json_data[i].width = dg.PKColumn._btnArray.length * 36;
                        }
                        ...
                        if (dg.isEditor && json_data[i].edit) {
                            ...
                            if (row && row.editor && row.editor.options && row.editor.type == "combobox") {
                                ...
                                //var xcommon = new xCommon();
                                //xcommon.FilterComboboxData(row.editor.options, row.editor.options.data, isrelaciton);
                            }
                        }
                        //格式化config表的数据结构
                        if (json_data[i].formatter && json_data[i].formatter.length > 2 && json_data[i].formatter.indexOf('#') != -1 && !/C_+/.test(json_data[i].formatter)) {
                            ...
                            json_data[i].formatter = "TY.Common.Formatter.configFormatter";
                        }
                        ...
                        if (json_data[i].formatter && json_data[i].formatter.length > 2 && json_data[i].formatter.indexOf('#') != -1 && /C_+/.test(json_data[i].formatter)) {
                            ...
                            json_data[i].formatter = "TY.Common.Formatter.objFormatter";
                        }

                        try {
                            if (!$.IsNullOrEmpty(json_data[i].formatter)) {
                                format = eval(json_data[i].formatter);
                            }
                        } catch (e) {
                            json_data[i].formatter = "TY.Common.Formatter." + json_data[i].formatter;
                        }
                        try {
                            format = eval(json_data[i].formatter);
                            if (format == undefined) {
                            ...
                            if (typeof (format) == "function") {
                                if (json_data[i].formatter == "TY.Common.Formatter.configFormatter") {
                                    json_data[i].formatter = format(configKey);
                                } else if (json_data[i].formatter == "TY.Common.Formatter.objFormatter") {
                    ...
                    //if (isMerge && megerLen == 0) {
                    //    json_data[i].rowspan = 2;
                    //}


                    if (json_data[i].frozen)// (i == 0)
                    {
                        frozen.push(json_data[i]);
                    }
                        //else if (json_data[i].rowspan > 1 || json_data[i].colspan > 1)
                        //{
                        //    if (json_data[i].rowspan > 1 && json_data[i].colspan <= 1) {
                        //        merge.push(json_data[i]);
                        //    } else if (json_data[i].colspan > 1) {
                        //        //这里我们只做一次赋值，因为后面的列都已经处理跨行
                        //        if (startIndex == 0) {
                        //            startIndex = json_data[i].ordernum;
                        //        }
                        //        isMerge = true; megerLen = json_data[i].colspan + 1;//因为当前列不计算，走到循环最后要减去一列
                        //        merge.push({ title: json_data[i].title, align: 'center', colspan: json_data[i].colspan, ordernum: json_data[i].ordernum });
                        //    }
                        //    else {
                    ...
                    //if (megerLen != 0/* && json_data[i].hidden != true || json_data[i].colspan > 1*/) {
                    //    megerLen = megerLen - 1;
                    //}
                    //configKey = null;
                }
                ...
                //        if (cols[i].ordernum < startIndex) {
                //            var currentItem = cols.splice(i, 1)[0];//得到合并列开始前的列                    
                //            currentItem.rowspan = 2;
                //            eachMerge: for (var j = 0; j < merge.length; j++) {
                //                if (merge[j].ordernum != undefined && currentItem.ordernum < merge[j].ordernum) {
                //                    merge.splice(j, 0, currentItem);
                //                    break eachMerge;
                //                }
                //            }
                //            currentItem = null;
            ...
            formatTGheader: function (header_json) {
                ...
                if (header_json) {
                    ...
                    if (_post_combox_data.length > 0) {
                        var _post_data = { item_json: JSON.stringify(_post_combox_data) };
                        TY.Global.m_combobox_json = TY.Global.m_combobox_json || [];
                        if (TY.combobox_params && $.type(TY.combobox_params) == "object") {
                            _post_data = $.extend({}, _post_data, TY.combobox_params);
                        }
                        TY.Global.m_combobox_json = TY.Global.m_combobox_json.concat(TY.Utility.Ajax.post("GetCombobox", "objName", _post_data, false));
        ...
        onDel: function (el, value, thatID, index) {
            var dg = ty.Global.DG.Items[thatID];
        ...
        */
        ...
        onOpen: function (el, value, thatID,index) {
            var dg = TY.Global.DG.operating = TY.Global.DG.Items[thatID]; //赋值当前对象到page属性方便调用
    ...
    function _createHtml(searchItem, dg) {
        ...
        if (searchItem.length > 0) {
            ...
            toolbarContainer.append(divSearchArea);
        }
        if (dg.Internal.type == "datagrid") {
            var div_fn = $('<div class="function-box" id="div_fun">');
            var item; actionKeys = actionKeys || "";
            ...
            //if (searchItem.length > 0) {
            //    item = $("<a>").append($('<input class=\"query\" flag=\"btn_query\" type=\"button\" name=\"查询\" value=\"\"/>'));
            //    div_fn.append(item);
            //}
            toolbarContainer.append(div_fn);
        }
        if (dg.Internal.type == "treegrid") {
        ...
        $("body").append(toolbarContainer); //加到页面中
        ...
        for (var i = 0, len = searchItem.length; i < len; i++) {
            ...
            if (i % 3 == 0) {
                line = $("<div class=\"line\">");
                $("#" + dg.Internal.toolbarID).find("[id='div_search']").append(line);
            }
            var div_item = $("<div class=\"short\">");
            var label = $("<label>").html(searchItem[i].title + "：");
            div_item.append(label);
            var input = $("<input type=\"text\"/>");
            //如果是下拉框模式执行以下代码
            var configKey;
            if ((typeof (searchItem[i].formatter) == "string" && searchItem[i].formatter.indexOf('#') != -1)) {
                if (/C_+/.test(searchItem[i].formatter)) {
                    objName = searchItem[i].formatter.split('#')[1];
                } else {
                    configKey = searchItem[i].formatter.split('#')[1];
                }
            }

            if (configKey || (objName && objName != '' && objName.indexOf('$') == -1)) {
                input = $("<select/>");
                input.attr("name", searchItem[i].field);
                if (configKey) {
                ...
                input.attr("onchange", "ty.Common._Internal.onQuery");
            }
            else {
                var dtype = searchItem[i].datatype.split(',');
                ...
                if (dtype[0] == "datetime") {
                    input.attr("name", searchItem[i].field).addClass(cssName).attr("date", true).width(150).attr("validType", "datebox");
                    if (searchItem[i].viewname && searchItem[i].viewname.indexOf('$1') != -1) {
                        input.width(95);
                    ...
                    input.attr("name", searchItem[i].field).addClass(cssName);
                    if (searchItem[i].viewname && searchItem[i].viewname.indexOf('$1') != -1) {
                        input.width(68);
                        var startInput = input.clone(true);
    ...
    function getObj(objName) {
        var obj = new Object();
        var m_combobox_json = ty.Global.m_combobox_json;
    ...
    function getNameByValue(obj, v) {
        ...
        if (v.toString().indexOf(',') != -1) {
            ...
            for (var i = 0; i < obj.length; i++) {
                if (ty.Utility.isInArray(array, obj[i]['value'])) {
    ...
    function getConfigName(configKey, value) {
        var items = ty.Global.config[configKey];
        var itemValue = [];
        if (value != undefined && value != null && value.toString() != '') {
            var valueArray = value.toString().split(',');
            var isIn = false;
            for (var i = 0; i < items.length; i++) {
                isIn = ty.Utility.isInArray(valueArray, items[i].value);
                if (!isIn) {
                    var iValue = items[i].value.toString();
                    if (iValue == "1" || iValue == "0") {
                        isIn = ty.Utility.isInArray(valueArray, iValue == "1" ? true : false);
                    }
                    else if (iValue == "true" || iValue == "false") {
                        isIn = ty.Utility.isInArray(valueArray, iValue == "true" ? 1 : 0);
                    }
                }
                if (isIn) {
    ...
    function getConfigValue(configKey, text) {
        ...
        if (text != undefined && text != null && text.toString() != '') {
            ...
            for (var i = 0; i < items.length; i++) {
                if (ty.Utility.isInArray(valueArray, items[i].text)) {
    ...
    function getDgByKey(key) {
        return ty.Global.DG.Items[key];
    }
    ty.Common = new Common();
    ...
    (function () {
        ...
        function bindComboboxByName(targetName, condition) {
            ...
            if (_obj[0]) {
                ...
                getComboboxData(item_data, condition);
        ...
        function bindCombobox(that) {
            ...
            if (configKey && ty.Global.config[configKey]) {
                ...
                var newItems = ty.Utility.gettree(items);
                ...
                if (!multiple && that.attr("defaultItem") != 'false') {
                    comboboxOption.data.unshift({ text: "请选择", id: "" });	//, value: ""
                }
                if (showTree) {
        ...
        function combobox(that, comboboxOption) {
            if (comboboxOption.multiple) {
                comboboxOption.onUnselect = function (node) {
                    var arrayValues = that.combotree("getValues");
                    var opts = that.combobox("options");
                    if (opts.editable && opts.multiple) {
                        fn_AutoComplete(that, null, true);
                    }
                    if (arrayValues.length == 0) {
                ...
                comboboxOption.onSelect = function (node) {
                    var arrayValues = that.combobox("getValues");
                    if (arrayValues[0] == '请选择') { arrayValues.shift(); }

                    var opts = that.combobox("options");
                    if (opts.editable && opts.multiple) {
                        fn_AutoComplete(that, null, true);
                    } else {
                        that.combobox("setValues", arrayValues);
                    }

                }
            }
            that.combobox(comboboxOption);
            if (that.combobox("options").data.length > 0) {
                if (that.combobox("options").data.length > 2 || that.attr('selectedIndex')) {
                    var selectedIndex = parseInt(that.attr('selectedIndex'));
                    var field = that.attr('unshowid') ? 'text' : 'value';
                    try {
                        that.combobox("select", that.attr("defaultItem") == 'false' ?
                            (selectedIndex ? that.combobox("options").data[selectedIndex - 1][field] : '请选择')
                            : (selectedIndex ? that.combobox("options").data[selectedIndex][field] : '请选择'));
                    }
                    catch (er) {
                        if (window.console) {
                    ...
                    //that.combobox("select", that.combobox("options").data[0].value == '' ? that.combobox("options").data[1].value : that.combobox("options").data[0].value);
                }
            }
        }

        function getComboboxData(item_data, condition) {
            //此判断是因为List跟Edit的请求方式不一样
            var m_combobox_json = ty.Global.m_combobox_json;
            if (condition == undefined && m_combobox_json.length > 0) {
                var _removeIndex = new Array();//需要移除的项
                for (var i = 0; i < ty.Global.m_combobox_json.length; i++) {
                ...
                if (_removeIndex.length > 0) {
                    for (var i = _removeIndex.length; i > 0 ; i--) {
                        item_data.splice(_removeIndex[i - 1], 1);
                    }
                }
            }
            //结束请求，开始渲染
            if (item_data.length > 0) {
                var _post_data = { item_json: JSON.stringify(item_data) };
                var result = Array();
                if (condition) {
                    _post_data = $.extend({}, _post_data, condition);
                }
                result = ty.Utility.Ajax.post("GetCombobox", "objName", _post_data, false);
                ...
                for (var i = 0, len = result.length; i < len; i++) {
                    for (var k in result[i]) {
                        var flag = true;
                        for (var ii = 0; ii < ty.Global.m_combobox_json.length; ii++) {
                            for (var kk in m_combobox_json[ii]) {
                                if (kk == k) {
                                    ty.Global.m_combobox_json[ii][kk] = result[i][k];
                                    flag = false;
                                }
                            }
                        }
                        if (flag == true) {
                            m_combobox_json = m_combobox_json.concat(result[i]);
                        }
                    }
                }

                if (m_combobox_json && !(m_combobox_json instanceof Array)) {
                    m_combobox_json = [m_combobox_json];
                }
                ty.Global.m_combobox_json = m_combobox_json;
            }
        }
        //绑定
        function bindObjNameCombobox(that) {
            ...
            if (m_combobox_json && m_combobox_json.length > 0) {
                ...
                current: for (var i = 0; i < ty.Global.m_combobox_json.length; i++) {
                ...
                var option = {
                    ...
                    onLoadSuccess: function () {
                        ...
                        //判断如果不是多选下拉就直接不选中，解决easyui的多选bug问题
                        if (!$target.combobox("options").multiple) {
                            $target.combobox("select", $target.attr("value") || "");
                        }
                ...
                if ($target.attr("multiple")) {
                    ...
                    if (typeof (eval(selFun)) == "function") {
                        option.onSelect = function (record) {
                ...
                option = $.extend(option, opts_change);
                //判断是否级联模式
                if ($("[parent='" + $target.attr('ObjName') + "']") && $("[parent='" + $target.attr('ObjName') + "']").length > 0) {
                    var option_extend = {
                        onSelect: function (record) {

                            //var $child = $("[parent='" + $target.attr("ObjName") + "']");
                            ...
                            $("[parent='" + $target.attr("ObjName") + "']").each(function () {
                                reBind(parent_id, $(this), m_combobox_json, option);
                            });
                            if ($("[parent='" + $target.attr('ObjName') + "']") && $("[parent='" + $target.attr('ObjName') + "']").length > 0) {
        ...
        //多级联下拉处理---------------------------------
        function reBind(parent_id, $element, ds, opts, parent) {
            var item_data; var new_data = [], unshowid = $element.attr("unshowid");
            leveal1: for (var i = 0; i < ds.length; i++) {
                //判断如果是不现实value值的模式，则需通过获取text对比拿到value之后去拿到子级联的数据
                if (unshowid) {
            ...
            option.editable = $element.attr("editable") == "true" ? true : false;
            //判断是否级联模式
                ...
                var option_extend = {
                    onSelect: function (record) {
                        ...
                        $("[parent='" + $element.attr("ObjName") + "']").each(function () {
                            reBind(parent_id, $(this), ty.Global.m_combobox_json, option);
                        });
                        if ($("[parent='" + $element.attr('ObjName') + "']") && $("[parent='" + $element.attr('ObjName') + "']").length > 0) {
        ...
        function triggerSelect($element) {
            //if ($("[parent='" + $element.attr('ObjName') + "']") && $("[parent='" + $element.attr('ObjName') + "']").length > 0) {
            //    var $child = $("[parent='" + $element.attr('ObjName') + "']");
            //    if ($child.attr('unshowid')) {
            //        $child.combobox("select", "请选择");
            //    } else if ($child.attr('multiple')) {
            //        $child.combobox("setText", "请选择");
            //    }
            //    else {
