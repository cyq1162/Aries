...
+function ($, ty) {
    ...
    function DataGrid(objName, tbName,id) {
        ...
        */
        ...
        this.Search = new TY.Common._Internal.Search();
        this.ToolBar = new TY.Common._Internal.ToolBar();
    }
    DataGrid.prototype = {        
        bind: bind
    }    
    function bind() {
        ...
        init(this);
        TY.Common._Internal.registerEvent(this);
        bindToolbar(this);
        ty.Global.DG.Items[this.Internal.id] = this;
    }
    function _edit_dbClick(rowIndex, rowData, dg) {
        ...
        if (dg.$target.datagrid('validateRow', dg.PKColumn.Editor.editIndex)) {
            var result = _editSave(dg, rowData, dg.PKColumn.Editor.editIndex);
            if (result) {
                _beginEditing(rowIndex, rowData, dg);
            }
        }
    ...
    function init(dg) {
        dg.Internal.headerData = ty.Utility.Ajax.post("GetHeader", dg.viewName,null,false);
        var objColumns = ty.Common.Formatter.formatHeader(dg);
        if (!objColumns) {
            return false;
        }
        var opts = dg.options;
        ...
        opts.url = (opts.url || TY.Utility.Ajax.Settings.url) + "?method=GetList&objName=" + dg.viewName + "&mid=" + mid + "&tableName=" + dg.tableName;
        var cfg = {
            toolbar: "#" + dg.Internal.toolbarID,
            loadMsg: "数据正在加载中...",
            idField: dg.Internal.idField,
            striped: true,
            nowwrap: false,
            singleSelect: true,
            checkOnSelect: false,
        ...
        function createColumnMenu() {
            ...
            dg.cmenu.menu({
                onClick: function (item) {
                    ty.Utility.Window.open(TY.handler.gridconfig + "?tbName=" + dg.viewName, "", false);
                }
            });
        ...
        if (costomToolbar == false && dg.isShowToolbar != false) {
            ty.Common._Internal.createSearchForm(dg); //创建SearchForm表单
            _setToolbar.call(dg,dg.ToolBar._btnArray);
        }

        opts = opts || {};
        if (opts.defaultWhere) {
            opts.queryParams = $.extend(opts.queryParams, { searchList: encodeURI(JSON.stringify(opts.defaultWhere)) });
        }
    ...
    */
    function _setButtons(btnItems) {
        ...
        each1: for (var i = 0; i < btnItems.length; i++) {
            ...
            , btn = $(_getBtnTemp(key))[0]//parseDom(getBtnTemp(key))[0]
        ...
        for (var i = 0, len = btnArray.length; i < len; i++) {
            ...
            if (!lv2action || actionKeys.indexOf(',' + lv2action) > -1) {                
                ...
                if (btn.html) {
                    ...
                    var title = btn.title;
                    item = ty.Utility.StringFormat.execute('<a><input class=\"{0}\" type=\"button\" onClick=\"{1}(event)\"  value=\"{2}\"/></a>', btnClass, btnClick, title);
                }
                ...
                if (count == 0) {
                    ...
                    toolbarContainer.children("a:eq(" + (index - 1) + ")").before(item);
                }
            }
        }
    }

    function bindToolbar(dg) {
        ...
        toolbar.delegate("[flag = 'btn_config']", "click", function () {
            var tbName = dg.viewName;
            ty.Utility.Window.open(TY.handler.gridconfig + "?tbName=" + tbName, "", false);
        });
        ...
        (function () {
            try {
                ...
                var exts = ["xls", "xlsx"];
                var url = ty.Utility.StringFormat.execute(ty.Utility.Ajax.Settings.url + '?objName={0}&tableName={1}&method=Import&mid={2}', dg.viewName, dg.tableName, mid);
                ty.Utility.initUploadButton(url, id, "excelImport", exts,
				function (file, ext) {
				    if (ty.Utility.isInArray(exts, ext)) {
				        $.messager.progress({
				            title: "消息提示",
				            msg: "正在导入数据，请稍候..."
				        });
				        ...
				        ty.Utility.Window.showMsg('上传文件扩展名必须是已下格式<br/>' + exts);
				        return false;
				    }
				    var param = {};
				...
				function (original_filename, data) {
				    ...
				    if (data.success) {
				        ty.Utility.Window.showMsg(data.msg);
				        ...
				        ty.Utility.Window.showMsg(tip, null, null, 8000);//"导入失败！"
				        if (data.downcode != undefined) {
				            //发起下载错误Excel文件。
				            //(fn, objName, data, async, type, url, callback) {
				            var ifrme = $("#div_ifrme_template");
				            ifrme && ifrme.remove();
				            ...
				            var form_export = $("<form>").attr("action", ty.Utility.Ajax.Settings.url).attr("target", iframeName);
				            form_export.append($("<input>").attr("name", "downcode").val(data.downcode));
				            form_export.append($("<input>").attr("name", "method").val("Down"));
				            form_export.append($("<input>").attr("name", "mid").val(mid));
                ...
                throw new Error("导入控件注册失败,请引入TY.Utility.js文件");
            }
        }())
    ...
    function PKColumn(dg) {
        ...
        *@param{string} key 指向TY.Common.js文件buttons_temp对象中的key值
        ...
        */
        ...
        this.onOpen = function (el, value, thatID,index) {
            ...
            var url = $aTarget.attr("url") || location.href.substring(location.href.lastIndexOf('/') + 1, splitIndex) + 'Edit.aspx';
            ...
            if (fn && typeof (fn) == "function") {
                ...
                TY.Utility.Window.open(url, (winTitle || " "), op == 1);
            }

        };
        this.onDel = function (el, value, dgID,index) {
            var dg = ty.Global.DG.Items[dgID];
            var ids = new Array();
            if (value) {
                dg.$target.datagrid("selectRecord", value);
                var row = dg.$target.datagrid("getSelected");
                var flag = dg.PKColumn.BtnDel.onBeforeExcute.call(el, value, row, index);
                if (flag == false) {
                    return false;
                }
                ids.push(value);//"'" + id + "'"
            } else {
                var selRows = dg.getCheckedIDs();
                if (selRows.length == 0) {
                    ty.Utility.Window.showMsg("请选择要删除的数据!");
                    return false;
                }
                ids = selRows;
            }
            ty.Utility.Window.confirm('确认删除操作吗？', null, function () {
                ty.Utility.Ajax.post("Delete", dg.tableName, { "id": ids.join(',') }, false, null, function (responseData) {
                    if (responseData.success != undefined && responseData.success) {
                        dg.PKColumn.BtnDel.onAfterExcute.call(el, value, row, index, responseData);
                        ty.Utility.Window.showMsg("删除成功");
                        dg.reload();
                    } else {
                        ty.Utility.Window.showMsg("删除失败！错误消息：" + responseData.msg);
                        dg.reload();
                    }
                });
            });
        };
        this.BtnDel = function () {
            function Obj() {
                ty.ExcuteEvent.call(this);
                /**
                *this 指向 HTML 元素
                */
                this.onBeforeExcute = function (value, row, index) { };
                /**
                *this 指向 HTML 元素
                *@param{responseData} 删除后返回的结果
                */
                this.onAfterExcute = function (value, row, index, responseData) { }
            }
            return new Obj();
        }();        
                    ...
                    this.onExcute = function (dg, value, index) {
                        if (endEditing(dg)) {
                            ...
                            if (dg.isEditor && dg.$target.datagrid("getSelected")[dg.Internal.primarykey]) {
                                TY.Utility.Window.confirm("确定删除此条信息吗？", null, function () {
                                    var result = TY.Utility.Ajax.post("Delete", dg.tableName, { id: value }, false, dg.options.url || TY.handler.ajax);
                                    if (result.success) {
                                        dg.$target.datagrid('deleteRow', index);
                                        dg.$target.datagrid('acceptChanges');
                                    }
                                    TY.Utility.Window
                                    TY.Utility.Window.showMsg(result.msg);
                ...
                function Obj() {
                    ...
                    */
                    ...
                    this.onExcute = function (dg, value, index) {
                        ...
                        var currentRow = dg.$target.datagrid("getSelected");
            ...
            if (row) {
                ...
                if (_change_data) {
                    if (dg.PKColumn.Editor.isSaveToBehind == false) {
                        ...
                        var post_data = getChangeJson(_change_data, row);
                        post_data[dg.Internal.primarykey] = row[dg.Internal.primarykey];//附加主键的ID值传入后台                                
                        ...
                        var result = TY.Utility.Ajax.post(dg.PKColumn.Editor.operator, dg.tableName, post_data, false, dg.options.url || TY.handler.ajax);
                        if (result.success) {
