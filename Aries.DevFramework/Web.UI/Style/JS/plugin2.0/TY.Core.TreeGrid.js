(function ($,ty) {
    ...
    TY.TreeGrid = function (ID, objName, tableName, idField, treeField, parentField, rootID, isExpand) {
        this.Internal = {
            type:'treegrid',
            id: ID || 'tg',
    ...
    TY.TreeGrid.prototype = {
        bind: function () {
            this.Internal.headerData = TY.Utility.Ajax.post('GetHeader', this.objName, null, false);
            var that = this;
            var cols = TY.Common.Formatter.formatTGheader(that.Internal.headerData);
            var _options = {
                iconCls: 'icon-edit',
                ...
                loadMsg: "",
                url: TY.handler.ajax
                    + TY.Utility.StringFormat.execute('?ObjName={0}&method={1}&parentField={2}&mid={3}',
                    that.objName, (that._searchSub ? 'GetList' : 'GetList')
                    , that.parentField, mid),
                fitColumns: true,
                singleSelect: true,
                columns: [cols],
                ...
                onSelect: function (row) {
                    ...
                    if (that.editRow) {
                        ...
                        if (_validChange(edites, that)) {
                            ...
                            TY.Utility.Ajax.post(that.operator, that.tableName, data, null, '/Ajax/TreegridHandler.ashx', function (d) {
                                var msg = '操作成功！';
                                if (d.success) {
            ...
            TY.Common._Internal.createSearchForm(that);
            that.$target = $("#" + that.Internal.id).treegrid(merge_options);
            _registerToolbar(that);
            TY.Common._Internal.registerEvent(that);
        }
    }
    ...
    function createRowMenu(tg_obj) {
        ...
        tg_obj.cmenu.menu('appendItem', {
            text: "移除", name: 'remove', onclick: function () {
                ...
                TY.Utility.Window.confirm('确认删除吗？', null, function () {
                    var obj = TY.ajax('Delete', tg_obj.tableName, { id: StringFormat.execute("{0}", id) }, false);
                    if (obj.success) {
                        tg.treegrid('remove', id);
                        //tg.treegrid("reload");
                    }
                    TY.Utility.Window.showMsg(obj.msg);
                });
    ...
    function createColumnMenu(tg_object) {
        ...
        tg_object.hmenu.menu({
            onClick: function (item) {
                TY.Utility.Window.open(TY.handler.gridconfig + "?tbName=" + tg_object.objName, "", false);
            }
        });
    ...
    function _registerToolbar(that) {
        ...
        toolbar.delegate("[flag = 'btn_add']", "click", function () {
            ...
            var $form = $("<form>");
            var colConstruct = that.headerData;
            for (var i in colConstruct) {
                var field = that.headerData[i].field;
                if (field == that.parentField || field == that.idField)
                {
                    continue;
                }
                var hidden = that.headerData[i].hidden;
                var title = that.headerData[i].title;
                
                var type = that.headerData[i].datatype;
                //var isnull = that.headerData[i].title;
            ...
            $('#' + div_id).dialog({
                ...
                toolbar: [{
                    ...
                    handler: function () {
                        ...
                        result = TY.ajax("Add", that.tableName, postData, false);
                        if (result && result.success) {
                            TY.Utility.Window.showMsg("添加成功", "消息提示");
                            tg.treegrid("reload");
                            $('#' + div_id).dialog("close");
                        }
                    }
