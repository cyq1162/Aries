...
/* by 卢建华 on 2013-11-29
 * 扩展vilidatebox的两个方法：移除验证和恢复验证
 */
    ...
    */
    valiUserName: {
        validator: function (value, param) {
            ...
            $.ajax({
                ...
                dateType: 'json',
                url: location.href,
                data: { UserName: value, method: "ValidateUserName" },
    ...
    RemoteValid: {
        validator: function (value, param) {
            var that = this;
            var bl = false;
            var url = param[1] || TY.handler.ajax;
            var op = param[2] || parent.TY.Page.operator;
            var param = param[0] || {};
            var post_data = { RemoteValue: value, op: op,method:'ValidFieldRepeat',id:Request.queryString('id') };
            for (var k in param) {
                post_data[k] = param[k];
            }
            $.ajax({
